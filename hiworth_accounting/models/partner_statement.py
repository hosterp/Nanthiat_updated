...
class partner_statement(models.Model):
    ...
    state = fields.Selection([('notposted','Not Posted'),
                            ('posted','Posted')])
...
#     move_lines = fields.One2many(related='account_id.move_lines',  string='Payments',)
    ...
    def action_post(self):
        ...
        print 'hhhhhhhhhhhh',journal_id
        for rec in self:
            move_vals = {'journal_id':journal_id.id,
                            'date':rec.date}
            for r in rec.statement_ids:
                
                move_line_vals = {
                            'account_id':rec.partner_id.property_account_receivable.id,
                            'credit':0,
                            'debit':r.amount,
                            'acc_balance':rec.partner_id.property_account_receivable.balance,
                            'name':r.name}
                move_line_list.append((0,0,move_line_vals))
                move_line_vals = {
                            'account_id':rec.account_id.id,
                            'credit':r.amount,
                            'debit':0,
                            'acc_balance':rec.account_id.balance,
                            'name':r.name}

                move_line_list.append((0,0,move_line_vals))
                print 'nnnnnnnnnnnnnnnnnnnnnnnnnnn',move_line_list
            move_vals.update({'line_id':move_line_list})
        #move_obj.create(move_vals)
        move_id = move_obj.create(move_vals)
        rec.write({'move_id': move_id.id, 'state': 'posted'})
