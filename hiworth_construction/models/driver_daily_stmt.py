...
class DieselPumpLine(models.Model):
	...
	date = fields.Date(related="line_id2.date")
...
class DriverDailyStatement(models.Model):
	...
	def get_transferred_amount1(self):
		records = self.env['cash.confirm.transfer'].sudo().search([('state','=','accepted'),('date','=',self.date),('name','=',self.driver_name.id)])
		amount = 0
		if records:
			for line in records:
				amount += line.amount
	...
	def compute_received_amount1(self):
		rec = self.env['cash.confirm.transfer'].sudo().search([('state','=','accepted'),('date','=',self.date),('user_id','=',self.driver_name.id)])
		received = 0
		if rec:
			for line in rec:
	...
	def get_transferred_amount(self):
		records = self.env['cash.confirm.transfer'].sudo().search([('date','=',self.date),('name','=',self.driver_name.id)])
		amount = 0
		if records:
			for line in records:
				amount += line.amount
	...
	def compute_received_amount(self):
		rec = self.env['cash.confirm.transfer'].search([('date','=',self.date),('user_id','=',self.driver_name.id)])
		received = 0
		if rec:
			for line in rec:
				received += line.amount
	...
	def create(self, vals):
		if vals.get('vehicle_no'):
			vals['start_km'] =self.env['fleet.vehicle'].browse(vals.get('vehicle_no')).odometer
	...
	def approve_entry(self):
		...
		if expenses != 0:
			move_expense = self.env['account.move'].create({'journal_id':journal.id,'driver_stmt_id':self.id,'date':self.date})
			move_line.create({
		...
		if self.cleaner_bata:
			move_cleaner_bata = self.env['account.move'].create({'journal_id':journal.id,'driver_stmt_id':self.id,'date':self.date})
		...
		if self.diesel_pump_line:
			for lines in self.diesel_pump_line:
				...
				move_diesel_pump = self.env['account.move'].create({'journal_id':journal.id,'driver_stmt_id':self.id,'date':self.date})
		...
		for line in self.driver_stmt_line:
			...
			move_from = self.env['account.move'].create({'journal_id':journal.id,'driver_stmt_id':self.id,'date':self.date})
		...
		if driver_betha != 0:
			move_rent = self.env['account.move'].create({'journal_id':journal.id,'driver_stmt_id':self.id,'date':self.date})

			shuttle_driver_amt = 0
			for shuttle in self.shuttle_service:
				if shuttle.shuttle_driver_amt != 0:
					move_line.create({'move_id':move_rent.id,
								  ...
								  'debit':shuttle.shuttle_driver_amt,
								  'credit':0,
							})
			...
			move_line.create({'move_id':move_rent.id,
							  ...
							  'period_id' : move_rent.period_id.id,
							  'debit':0,
							  'credit':driver_betha + shuttle_driver_amt,
						})
			move_line.create({'move_id':move_rent.id,
							  'state': 'valid',
							  'name': 'Rent Percent',
		...
		if self.withdrawal_amount != 0:
			move_withdrawal_amount = self.env['account.move'].create({'journal_id':journal.id,'driver_stmt_id':self.id,'date':self.date})
		...
		if self.received != 0:
			move_driver_petty = self.env['account.move'].create({'journal_id':journal.id,'driver_stmt_id':self.id,'date':self.date})
