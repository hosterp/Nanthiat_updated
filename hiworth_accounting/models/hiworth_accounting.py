...
#    is_move_lines = fields.Boolean(compute='_compute_move_lines', store=True, string='Is Journal Items')
 ...
 #   lia_ids = fields.Many2one('account.account', compute='get_liability_ids', store=True, string="Child Accounts")
    ...
    expense_ids = fields.One2many('account.account', 'many_id3',  store=True, string='Expenses')
...
class res_partner(models.Model):
    ...
    property_account_receivable = fields.Many2one('account.account','Account Receivable',
            ...
            required=True)
    company_expense_id = fields.Many2one('account.account','Company Expense Account')
...
class account_voucher(models.Model):
    ...
    partner_daily_stmt_id = fields.Many2one('partner.daily.statement', string="SDS")
    driver_daily_stmt_id = fields.Many2one('driver.daily.statement', string="DDS")
    state = fields.Selection(
            [('draft','Draft'),
             ('cancel','Cancelled'),
             ('proforma','Pro-forma'),
             ('posted','Posted'),
             ('approved', 'Approved')], 'Status', readonly=True, track_visibility='onchange', copy=False,
