...
class work_order_line(models.Model):
    ...
    def _compute_amount(self):
        for line in self:
            ...
            line.amt_subtot = line.qty * line.rate
            line.tax_amt = line.amt_subtot * line.tax_id.amount

            if line.tax_id.price_include == True:
                line.amount = line.amt_subtot - line.tax_amt
            else:
                line.amount = line.amt_subtot + line.tax_amt
    ...
    product_id = fields.Many2one('product.product', string='Name of Item', required=True)
    rate = fields.Float('Rate', required=True)
    qty = fields.Float('Qty', default=1)
    uom_id = fields.Many2one('product.uom', 'Unit', required=True)
    tax_id = fields.Many2one('account.tax',string="GST")
    tax_amt = fields.Float(string='Tax Amount', compute='_compute_amount')
    amt_subtot = fields.Float(string='Amount Subtotal', compute='_compute_amount')
...
class work_order(models.Model):
    ...
    template_id = fields.Many2one('work.order',string="Template")
    is_template = fields.Boolean(string="Is Template") 
    is_agreement = fields.Boolean(string="Is Agreement", default=False)
    is_workorder = fields.Boolean(string="Is Work Order", default=False)
...
#     item_ids = fields.One2many()
    ...
    def onchange_template_id(self):
        if self.template_id:
            l1 = []
            d1 = {}
            for val in self.template_id.order_lines:
                d1 = {
            ...
            self.part1 = self.template_id.part1
            self.scope = self.template_id.scope
            self.terms = self.template_id.terms
            self.order_lines = l1
